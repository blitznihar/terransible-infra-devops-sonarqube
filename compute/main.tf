...
resource "aws_instance" "aws_instance_web" {
    ...
    provisioner "local-exec" {
        command = "sleep 120; ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u centos --private-key ~/.ssh/id_rsa -i '${aws_instance.aws_instance_web.public_ip},' ./ansible/sonarqube_main.yml"
    }
}